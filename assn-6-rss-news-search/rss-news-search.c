...
typedef struct {
  sem_t http_conn_limit;
  sem_t seen_search_lock;
  sem_t add_word_lock;
  sem_t create_server_sem_lock;
  hashset server_sems;
} semaphores;
...
static void InitSemaphores(rssDatabase *db);
static void DestroySemaphores(rssDatabase *db);
static void BuildIndices(rssDatabase *db);
...
static void ParseArticle(thread_data_t *data);
static void ScanArticle(rssDatabase *db, streamtokenizer *st, int articleID, hashset *indices, hashset *stopWords);
...
static int SemHash(const void *elem, int numBuckets);
static int SemCompare(const void *elem1, const void *elem2);
...
static const char *const kWelcomeTextFile = "/home/suvov/CS107/A6/data/welcome.txt";
static const char *const kDefaultStopWordsFile = "/home/suvov/CS107/A6/data/stop-words.txt";
static const char *const kDefaultFeedsFile = "/home/suvov/CS107/A6/data/rss-feeds-techy.txt";
...
static void InitSemaphores(rssDatabase *db)
{
  sem_init(&db->sems.seen_search_lock, 0, 1);
  sem_init(&db->sems.http_conn_limit, 0, 24);
  sem_init(&db->sems.add_word_lock, 0, 1);
  sem_init(&db->sems.create_server_sem_lock, 0, 1);
  HashSetNew(&db->sems.server_sems, sizeof(server_sem_t), kNumStopWordsBuckets, SemHash, SemCompare, SemFree); 
}

//
static void DestroySemaphores(rssDatabase *db)
{
  if(sem_destroy(&db->sems.seen_search_lock) != 0) printf("Error destroying semaphore");
  if(sem_destroy(&db->sems.http_conn_limit) != 0) printf("Error destroying semaphore");
  if(sem_destroy(&db->sems.add_word_lock) != 0) printf("Error destroying semaphore");
  if(sem_destroy(&db->sems.create_server_sem_lock) != 0) printf("Error destroying semaphore");
  //int size = HashSetCount(&db->sems.server_sems);
  //printf("Created server semaphores:%d\n", size);
  HashSetDispose(&db->sems.server_sems);
}
...
static void BuildIndices(rssDatabase *db)
{
    ...
    VectorNew(&db->articlesToParse, sizeof(rssNewsArticle), NewsArticleFree, 0); //vector with articles to be parsed
    STNew(&st, infile, kNewLineDelimiters, true);
    ...
    fclose(infile);
    int n, i, j;
    n = VectorLength(&db->articlesToParse);
    pthread_t threads[n];
    thread_data_t data[n];
    for(i = 0; i < n; i++) {
      rssNewsArticle article = *(rssNewsArticle *)VectorNth(&db->articlesToParse, i);
      data[i].db = db;
      data[i].articleTitle = article.title;
      data[i].articleURL = article.fullURL;
      data[i].redirected = false;
      if(pthread_create(&threads[i], NULL, (void *) &ParseArticle, (void *) &data[i]) != 0) printf("Error creating thread!\n");
    }
    for(j = 0; j < n; j++)
      if(pthread_join(threads[j], NULL) != 0) printf("Error joining thread!\n"); // join all threads to main    
    VectorDispose(&db->articlesToParse); // don't need it anymore
}
...
static void ProcessEndTag(void *userData, const char *name)
{
  ...
  if (strcasecmp(name, "item") == 0) {	
    rssNewsArticle newsArticle;
    NewsArticleClone(&newsArticle, entry->title, "\0", entry->url);
    VectorAppend(&state->db->articlesToParse, &newsArticle);	
  }
}
...
//
{
  rssDatabase *db = data->db; 
  const char *articleTitle = data->articleTitle; 
  const char *articleURL = data->articleURL; 
  url u;
  urlconnection urlconn;
  streamtokenizer st;
  int articleID;
  ...
  sem_wait(&db->sems.seen_search_lock); // wait
  int result = VectorSearch(&db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false);
  if (result >= 0) {
    printf("[Ignoring \"%s\": we've seen it before.]\n", articleTitle);
    URLDispose(&u); 
    sem_post(&db->sems.seen_search_lock); // release
    return; 
  }
  sem_post(&db->sems.seen_search_lock); // release
  sem_wait(&db->sems.http_conn_limit); // wait until one of possible connections available
  URLConnectionNew(&urlconn, &u);
  
  sem_wait(&db->sems.create_server_sem_lock);
  server_sem_t *current_server_sem = malloc(sizeof(server_sem_t));
  strcpy(current_server_sem->server_name, u.serverName);
  void *found = HashSetLookup(&db->sems.server_sems, &current_server_sem);
  if(found == NULL){
    //printf("Creating semaphore for %s\n",  current_server_sem->server_name);
    sem_init(&current_server_sem->semaphore, 0,6);
    HashSetEnter(&db->sems.server_sems, &current_server_sem);
  } else {
    free(current_server_sem);
    current_server_sem = *(server_sem_t **)found;
    //printf("No need in creating semaphore for %s\n",  current_server_sem->server_name);
  }
  sem_post(&db->sems.create_server_sem_lock);
  sem_wait(&current_server_sem->semaphore); // wait for available connection to given server
  ...
  switch (urlconn.responseCode) {
      ...
      case 200: printf("[%s] Indexing \"%s\"\n", u.serverName, articleTitle);
	        sem_wait(&db->sems.seen_search_lock); // wait
		result = VectorSearch(&db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false);
		if(result >= 0) { // if article was added in meantime
		  sem_post(&db->sems.seen_search_lock); // release
		  break;
		}
		...
		ScanArticle(db, &st, articleID, &db->indices, &db->stopWords);
		STDispose(&st);
		break;
      case 301: 
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
	        if(data->redirected) { // redirected second time, something wrong
		 sem_post(&current_server_sem->semaphore); // release server
		 sem_post(&db->sems.http_conn_limit); // release connection
		 URLConnectionDispose(&urlconn);	
		 URLDispose(&u);
		 printf("Redirected second time! Returning.\n");
		 return;
	        }
	        data->redirected = true;
	        newurl = strdup(urlconn.newUrl);
	        data->articleURL = newurl;
		printf("Redirecting to :%s\n", data->articleURL);
		sem_post(&current_server_sem->semaphore); // release server
		sem_post(&db->sems.http_conn_limit); // release connection
		URLConnectionDispose(&urlconn);	
		URLDispose(&u);
		ParseArticle(data); // now trying to parse with updated url
		StringFree(&data->articleURL);
		return; // released all stuff so free to return just here
...
// semaphores hashing by server name(char *)
static int SemHash(const void *elem, int numBuckets)
{
  server_sem_t *server_sem = *(server_sem_t **) elem;
  unsigned long hashcode = 0;
  const char *s = server_sem->server_name;
...
// comparing by server name
static int SemCompare(const void *elem1, const void *elem2)
{
  server_sem_t *server_sem1 = *(server_sem_t **) elem1;
  server_sem_t *server_sem2 = *(server_sem_t **) elem2;
  const char *server_name1 = server_sem1->server_name;
  const char *server_name2 = server_sem2->server_name;
  return strcmp(server_name1, server_name2);
}
static void SemFree(void *elem)
{
  server_sem_t *server_sem = *(server_sem_t **) elem;
  sem_destroy(&server_sem->semaphore);
  free(server_sem);
}
